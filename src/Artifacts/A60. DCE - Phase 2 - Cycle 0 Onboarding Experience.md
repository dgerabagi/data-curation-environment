...
# Updated on: C155 (Clarify generic artifact naming)
...
-   **Detection (`history.service.ts`):** Unchanged. `getLatestCycle` returns a `cycleId: 0` object if `A0. Master Artifact List.md` is missing.
-   **Frontend (`OnboardingView.tsx`):** The "Generate" button will send `RequestCreateCycle0Prompt` with the project scope payload. A "Continue to Cycle 1" button appears after generation is complete.
-   **Backend (`prompt.service.ts`):** `generateCycle0Prompt` will be refactored. It will read only files from the extension's packaged `src/Artifacts` directory that begin with `T` to build the static context. It will create `A0. Master Artifact List.md` without any project-specific prefix.
