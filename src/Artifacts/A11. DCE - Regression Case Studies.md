...
# Updated on: C139 (Add Workflow Highlight Race Condition case)
...
### Case Study 026: Animated Workflow Highlights Wrong Step on New Cycle

-   **Artifacts Affected:** `view.tsx`, `useWorkflow.ts`
-   **Cycles Observed:** C136-C139
-   **Symptom:** Immediately after clicking the `+` button to create a new cycle, the animated workflow guide highlights the "Generate prompt.md" button instead of the first response tab ("Paste Response").
-   **Root Cause Analysis (RCA):** A race condition existed between the `useWorkflow` hook's transition logic and its initialization logic. When the cycle changed, the `workflowStep` state from the previous cycle (`readyForNewCycle`) persisted for a render cycle. In the context of the *new* cycle, the criteria for `readyForNewCycle` (i.e., `isReadyForNextCycle`) were false. The transition logic, seeing `readyForNewCycle` but `!isReady`, correctly fell back to `awaitingGeneratePrompt`. This happened before the initialization effect could run to reset the step to `awaitingResponsePaste_1`, causing the workflow to get stuck at the wrong step.
-   **Codified Solution & Best Practice:**
    1.  When performing a major context switch (like creating a new cycle) that invalidates the current workflow state, explicitly reset the workflow state in the event handler itself (`setWorkflowStep('awaitingResponsePaste_1')`).
    2.  Do not rely solely on `useEffect` to reactively reset state when dependencies change if there are competing effects that might transition the state based on the *old* value during the render pass. Explicit, imperative resets in the handler are safer for these transitions.
