...
# Updated on: C78 (Add case for special file type caching failure on initialization)
...
-   **Cycles Observed:** 65, 66, 75, 76, 78 (Recurring issue with PDF, then Excel, then Word)
...
-   **Root Cause Analysis (RCA):** The frontend logic was correctly identifying the restored paths but was failing to dispatch the necessary processing requests to the backend *before* the user took an action. The `useEffect` hook responsible for handling the `ApplySelectionSet` message would set the `checkedFiles` state, but the subsequent loop to check file extensions and send IPC messages (`RequestPdfToText`, etc.) was either flawed, operating on stale data, or not being triggered reliably. This created a race condition where the backend cache remained empty for any restored selection.
-   **Codified Solution & Best Practice:**
    1.  **Robust Pre-warming:** The `useEffect` hook in `context-chooser.view/view.tsx` that listens for the `ApplySelectionSet` message is the single point of truth for this logic.
    2.  This hook must iterate through the **complete list of paths** received from the backend.
    3.  For each path, it must perform a simple check on its file extension.
    4.  Based on the extension, it **must** dispatch the correct IPC message (`RequestPdfToText`, `RequestExcelToText`, `RequestWordToText`) to the backend *immediately* upon receiving the restored selection.
    5.  **Logging is critical:** Add detailed client-side logs to this loop to confirm which paths are being checked and which IPC messages are being sent. Add corresponding server-side logs to the handlers in `fs.service.ts` to confirm receipt of these messages. This makes debugging future failures trivial. This process ensures the backend cache is always populated before any user action.
